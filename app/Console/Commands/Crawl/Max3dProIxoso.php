...
class Max3dProIxoso extends BaseCommand
{
    use CommonCrawl;

    protected $signature = 'crawler:max3d-pro {--latest : Chỉ crawl kỳ quay mới nhất}';
    protected $description = 'cào ixoso max3d pro';
    ...
    protected function executeCommand(): void
    {
        ...
        $sources = $isLatest
            ? ['https://ixoso.com/xo-so-tu-chon-max3d-pro.html']
            : [
                'https://ixoso.com/kq-xo-so-max3dpro-thu-3.html',
                'https://ixoso.com/kq-xo-so-max3dpro-thu-5.html',
                'https://ixoso.com/kq-xo-so-max3dpro-thu-7.html',
            ];
    ...
    private function detectSection(Crawler $section): array
    {
        ...
        $textPeriod = $section->filter('table:not(.table-result) b')->text();
        preg_match('#\#(\d+)#', $textPeriod, $period);
...
//        number
        ...
        $section->filter('table:not(.table-result) td:nth-child(2)')->each(function (Crawler $td, $k) use (&$details){
            $key = match ($k){
